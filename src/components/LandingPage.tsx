...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  return (
    <div className="w-full relative overflow-hidden pb-24 text-white min-h-screen bg-[#4D44FF]">
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-[600px] bg-[#3823D9]/40 rounded-full blur-[160px]" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10 pt-8 sm:pt-12 space-y-20">
        {/* 1. EDITORIAL POSTER HERO SECTION (DIRECTLY MATCHING REFERENCE IMAGE) */}
        <section className="text-center space-y-6 max-w-5xl mx-auto">
          {/* Top Micro-Labels & Pill Badge Header */}
          <div className="flex items-center justify-between text-[10px] sm:text-xs font-mono-micro tracking-widest uppercase text-white/90 border-b border-white/20 pb-4">
            <div className="text-left font-bold">
              <div>PITCH STUDIO</div>
              <div className="text-white/60">AUTONOMOUS</div>
            </div>
            ...
            <div className="text-right font-bold">
              <div>MULTI-AGENT</div>
              <div className="text-white/60">+VC CRITIC</div>
            </div>
          </div>

          {/* Massive Editorial Display Title in Pastel Blush Pink */}
          <div className="pt-4 space-y-2">
            <h1 className="font-poster text-6xl sm:text-8xl lg:text-9xl tracking-tight text-[#FFCCD5] leading-[0.85] uppercase drop-shadow-sm">
              PITCH<br />
              ARCHITECT
            </h1>
          </div>
          ...
          <div className="flex items-center justify-between text-[9px] sm:text-[11px] font-mono-micro tracking-widest uppercase text-white/80 py-2 border-y border-white/20">
            <div>MULTI AGENT</div>
            <div>LANGUAGE SUPPORT EN/GLOBAL</div>
            <div>INSTITUTIONAL GRADE</div>
          </div>

          {/* Elegant Centered Description */}
          <p className="text-sm sm:text-base text-white/90 max-w-2xl mx-auto leading-relaxed font-normal pt-1">
          ...
          <div className="max-w-2xl mx-auto pt-3">
            <div className="p-2.5 rounded-2xl border border-white/30 bg-[#2A15C2]/80 backdrop-blur-xl shadow-2xl flex flex-col sm:flex-row items-center gap-2">
              <div className="relative flex-1 w-full">
                ...
                <input
                  ...
                  placeholder="Describe your startup idea (e.g. AI platform for cold-chain pharma logistics)..."
                  className="w-full bg-[#3823D9] border border-white/25 rounded-xl pl-10 pr-4 py-3 text-xs sm:text-sm text-white placeholder-white/50 focus:outline-none focus:border-[#FFCCD5] transition-colors"
                />
              </div>

              <button
                onClick={handleLaunch}
                className="w-full sm:w-auto px-7 py-3.5 rounded-xl font-bold text-xs sm:text-sm btn-pink-primary shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 whitespace-nowrap"
              >
                <Zap className="w-4 h-4 text-[#2612B0]" />
          ...
          {/* 2. LIVE DYNAMIC INTERACTIVE PLAYGROUND (Real-Time Counter Simulation) */}
          <div className="pt-6 sm:pt-10 perspective-1400 max-w-5xl mx-auto">
            {/* Concept Switcher Tabs */}
            <div className="flex items-center justify-center gap-2 mb-4 flex-wrap">
              <span className="text-xs font-mono-micro font-bold text-[#FFCCD5] uppercase tracking-wider mr-1 flex items-center gap-1.5">
                <Activity className="w-3.5 h-3.5 text-[#FFCCD5]" /> Live Benchmark Demo:
              </span>
              {DYNAMIC_PLAYGROUND_PRESETS.map((preset, idx) => (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 ${
                    activePresetIdx === idx
                      ? 'bg-[#FFCCD5] text-[#2612B0] font-bold shadow-md scale-105'
                      : 'bg-[#2A15C2]/80 text-white/80 hover:text-white border border-white/20 hover:bg-[#3823D9]'
                  }`}
                >
                  <span className={`w-2 h-2 rounded-full ${activePresetIdx === idx ? 'bg-[#2612B0] animate-pulse' : 'bg-white/40'}`} />
                  {preset.name}
                </button>
              ))}
            </div>
            ...
            <div
              ...
              className="transform-style-3d relative rounded-3xl p-1 bg-[#3823D9]/60 border border-white/30 shadow-2xl cursor-pointer text-left"
            >
              <div className="rounded-[22px] bg-[#2A15C2]/95 p-5 sm:p-7 border border-white/25 overflow-hidden relative transform-style-3d space-y-5">
                {/* Top Card Bar */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/20 pb-4">
                  <div>
                    <div className="flex items-center gap-2.5">
                      <h3 className="font-display text-2xl font-extrabold text-[#FFCCD5]">{activePreset.name}</h3>
                      <span className="text-[10px] font-mono-micro px-2.5 py-0.5 rounded-full bg-[#3823D9] text-[#FFCCD5] font-bold border border-[#FFCCD5]/40">
                        {activePreset.vertical}
                      </span>
                    </div>
                    <p className="text-xs text-white/80 mt-1">{activePreset.inputData.businessIdea}</p>
                  </div>

                  {/* Live Dynamic Score Meter Badge */}
                  <div className="flex items-center gap-2 bg-[#3823D9] border border-white/30 px-3.5 py-1.5 rounded-xl shadow-sm self-start sm:self-auto">
                    <TrendingUp className="w-4 h-4 text-[#FFCCD5]" />
                    <div>
                      <div className="text-[9px] uppercase font-mono-micro font-bold text-white/70 tracking-wider">Investor Readiness</div>
                      <div className="text-sm font-extrabold text-white">
                        <span className="text-white/60 font-normal">{activePreset.initialScore}</span> →{' '}
                        <span className="text-[#FFCCD5] font-bold">{displayScore}/100</span>{' '}
                        <span className="text-[10px] text-[#FFCCD5] font-semibold">(+{activePreset.revisedScore - activePreset.initialScore} pts)</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 rounded-xl bg-[#3823D9] border border-white/20 font-mono-micro text-[11px] space-y-2.5 shadow-xl">
                    <div className="text-[#FFCCD5] font-bold flex items-center justify-between text-xs border-b border-white/20 pb-2">
                      <span className="flex items-center gap-1.5">
                        <Terminal className="w-3.5 h-3.5 text-[#FFCCD5]" /> Studio Workflow Log
                      </span>
                      {isSimulating && <RefreshCw className="w-3 h-3 text-[#FFCCD5] animate-spin" />}
                    </div>

                    <div className="text-white/80 text-[10px] leading-relaxed">
                      <span className="text-[#FFCCD5] font-semibold">Market Grounding:</span> Benchmark CAGR {activePreset.cagr}
                    </div>

                    <div className="text-white/80 text-[10px] leading-relaxed">
                      <span className="text-[#FFCCD5] font-semibold">Narrative Structure:</span> {activePreset.agentThought}
                    </div>

                    <div className="text-white/80 text-[10px] leading-relaxed">
                      <span className="text-white font-semibold">Partner Objection:</span> {activePreset.criticObjection}
                    </div>

                    <div className="p-2.5 rounded-lg bg-[#2A15C2] border border-[#FFCCD5]/40 text-white text-[10px] leading-tight">
                      <span className="font-bold text-[#FFCCD5]">Remediation:</span> {activePreset.resolution}
                    </div>
                  </div>

                  {/* Center & Right Column: Market Sizing Counters */}
                  <div className="md:col-span-2 p-5 rounded-xl bg-[#3823D9] border border-white/20 space-y-4 shadow-xl">
                    <div className="flex items-center justify-between">
                      <span className="pill-badge text-[10px] py-0.5 px-2.5">
                        MARKET SIZING BREAKDOWN (TAM / SAM / SOM)
                      </span>
                      <span className="text-[10px] text-white/70 font-mono-micro">Bottom-Up Unit Arithmetic</span>
                    </div>

                    {/* Animated Number Counters */}
                    <div className="grid grid-cols-3 gap-2.5 text-center">
                      <div className="p-3.5 rounded-xl bg-[#2A15C2] border border-white/20">
                        <span className="text-[10px] text-white/70 block font-bold font-mono-micro uppercase tracking-wider">TAM</span>
                        <div className="text-xl sm:text-2xl font-extrabold text-white mt-1">${displayTam}B</div>
                        <span className="text-[9px] text-white/60">Total Available</span>
                      </div>

                      <div className="p-3.5 rounded-xl bg-[#2A15C2] border border-white/20">
                        <span className="text-[10px] text-white/70 block font-bold font-mono-micro uppercase tracking-wider">SAM</span>
                        <div className="text-xl sm:text-2xl font-extrabold text-white mt-1">${(displayTam * (activePreset.sam / activePreset.tam)).toFixed(1)}B</div>
                        <span className="text-[9px] text-white/60">Serviceable Segment</span>
                      </div>

                      <div className="p-3.5 rounded-xl bg-[#2A15C2] border border-[#FFCCD5]/40 shadow-sm">
                        <span className="text-[10px] text-[#FFCCD5] block font-bold font-mono-micro uppercase tracking-wider">SOM</span>
                        <div className="text-xl sm:text-2xl font-extrabold text-[#FFCCD5] mt-1">${displaySom}B</div>
                        <span className="text-[9px] text-white/80">Beachhead Target</span>
                      </div>
                    </div>

                    {/* Dynamic Unit Multiplication Footnote */}
                    <div className="p-3 rounded-lg bg-[#2A15C2] border border-white/20 text-[11px] text-white/90">
                      <strong className="text-[#FFCCD5]">Bottom-Up Formula: </strong>
                ...
                <div className="pt-2 flex items-center justify-between flex-wrap gap-3 border-t border-white/20">
                  <span className="text-xs text-white/80 font-medium">Ready to build your own custom deck?</span>
                  <button
                    onClick={() => onLaunchStudio()}
                    className="btn-pink-primary px-5 py-2 text-xs flex items-center gap-1.5 shadow-md transition-all"
                  >
        ...
        <section className="space-y-8">
          <div className="text-center space-y-2 max-w-2xl mx-auto">
            ...
            <h2 className="font-display text-3xl sm:text-4xl font-extrabold text-[#FFCCD5]">
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
              <button
                ...
                className={`p-3.5 rounded-2xl border text-left transition-all ${
                  activeAgentNode === 'researcher'
                    ? 'bg-[#FFCCD5] text-[#2612B0] shadow-lg border-[#FFCCD5] font-bold'
                    : 'bg-[#2A15C2]/80 border-white/20 text-white/80 hover:text-white hover:bg-[#3823D9]'
                }`}
              >
                <div className="text-[10px] font-mono-micro uppercase opacity-80">Stage 01</div>
              ...
              >
                <div className="text-[10px] font-mono-micro uppercase opacity-80">Stage 02</div>
              ...
              >
                <div className="text-[10px] font-mono-micro uppercase opacity-80">Stage 03</div>
              ...
              >
                <div className="text-[10px] font-mono-micro uppercase opacity-80">Stage 04</div>
            ...
            <div className="rounded-3xl p-6 sm:p-8 bg-[#2A15C2] border border-white/30 shadow-2xl">
              {activeAgentNode === 'researcher' && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#FFCCD5]" />
                    <h3 className="font-display text-xl font-extrabold text-[#FFCCD5]">Market Intelligence &amp; Bottom-Up Sizing</h3>
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-[#3823D9] border border-white/20 font-mono-micro text-[11px] text-white">
                    Calculated TAM: <span className="text-[#FFCCD5] font-bold">${activePreset.tam}B</span> | SAM: <span className="text-[#FFCCD5] font-bold">${activePreset.sam}B</span> | SOM: <span className="text-[#FFCCD5] font-bold">${activePreset.som}B</span> ({activePreset.somUnits})
                  </div>
                </div>
              )}
              ...
              {activeAgentNode === 'narrative' && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#FFCCD5]" />
                    <h3 className="font-display text-xl font-extrabold text-[#FFCCD5]">10-Slide Narrative Blueprint</h3>
                  </div>
                  <p className="text-xs text-white/90 leading-relaxed">
              ...
              {activeAgentNode === 'critic' && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#FFCCD5]" />
                    <h3 className="font-display text-xl font-extrabold text-[#FFCCD5]">VC Partner Audit &amp; Rubric Evaluation</h3>
                  </div>
                  <p className="text-xs text-white/90 leading-relaxed">
                  ...
                  <div className="p-3 rounded-xl bg-[#3823D9] border border-white/20 font-mono-micro text-[11px] text-white">
                    Baseline Score: <span className="text-[#FFCCD5] font-bold">{activePreset.initialScore}/100</span> | Primary Flag: <span className="text-white">{activePreset.criticObjection}</span>
                  </div>
                </div>
              )}
              ...
              {activeAgentNode === 'refiner' && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#FFCCD5]" />
                    <h3 className="font-display text-xl font-extrabold text-[#FFCCD5]">Iterative Refinement &amp; Objection Resolution</h3>
                  </div>
                  <p className="text-xs text-white/90 leading-relaxed">
                  ...
                  <div className="p-3 rounded-xl bg-[#3823D9] border border-white/20 font-mono-micro text-[11px] text-white">
                    Post-Audit Score: <span className="text-[#FFCCD5] font-bold">{activePreset.revisedScore}/100</span> | Status: <span className="text-[#FFCCD5] font-bold">Ready for Partner Meeting</span>
                  </div>
                </div>
              )}
            </div>
              ...
              <div
                ...
                className={`p-5 rounded-3xl border cursor-pointer transition-all flex flex-col justify-between ${
                  activePersonaDrill === p.id
                    ? 'bg-[#2A15C2] border-[#FFCCD5] shadow-2xl scale-[1.02] ring-1 ring-[#FFCCD5]'
                    : 'bg-[#3823D9]/70 border-white/20 hover:border-white/40 hover:bg-[#3823D9]'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-[#FFCCD5]">{p.name}</span>
                    <span className="text-[9px] font-mono-micro font-bold px-2 py-0.5 rounded-full bg-[#2A15C2] text-white border border-white/30">
                      {p.id === 'seed_bull' ? 'Seed' : p.id === 'saas_skeptic' ? 'Series A' : p.id === 'deeptech_specialist' ? 'DeepTech' : 'Corporate'}
                    </span>
                  </div>
                  <div className="text-[11px] text-white/70 mb-3">{p.title} • {p.firmType.split('(')[0].trim()}</div>
                  <p className="text-xs text-white/90 leading-relaxed line-clamp-3">{p.evaluationBias}</p>
                </div>

                <div className="mt-4 pt-3 border-t border-white/20 text-[10px] text-[#FFCCD5] font-mono-micro">
        ...
        <section className="rounded-3xl p-8 sm:p-14 text-center space-y-6 bg-[#2A15C2] border border-white/30 relative overflow-hidden shadow-2xl">
          <div className="max-w-2xl mx-auto space-y-3">
            <h2 className="font-poster text-5xl sm:text-7xl font-extrabold text-[#FFCCD5] uppercase tracking-wide">
              READY TO PITCH WITH CONVICTION?
            </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
            <button
              onClick={() => onLaunchStudio()}
              className="w-full sm:w-auto px-9 py-4 rounded-xl font-bold text-sm btn-pink-primary shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
            >
              <Sparkles className="w-4 h-4 text-[#2612B0]" />
              <span>Open Studio Workspace</span>
              <ArrowRight className="w-4 h-4" />
            </button>
            <button
              onClick={onExploreReferenceDecks}
              className="w-full sm:w-auto px-7 py-4 rounded-xl font-bold text-sm btn-cobalt-secondary transition-all flex items-center justify-center gap-2"
            >
